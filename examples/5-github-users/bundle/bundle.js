...
var API_URL = 'http://api.github.com/users';

var refreshButtonStream = _rx2['default'].Observable.fromEvent(document.querySelector('.refresh'), 'click');
var refreshUser1Stream = _rx2['default'].Observable.fromEvent(document.querySelector('.refresh-user1'), 'click');
var refreshUser2Stream = _rx2['default'].Observable.fromEvent(document.querySelector('.refresh-user2'), 'click');

var apiUrlStream = _rx2['default'].Observable.just(API_URL);

var requestOnRefreshStream = refreshButtonStream.map(function (ev) {
  return API_URL + '?since' + Math.floor(Math.random() * 500);
});

var responseStream = apiUrlStream.merge(requestOnRefreshStream).flatMap(function (url) {
  return _rx2['default'].Observable.fromPromise(_jquery2['default'].getJSON(url));
}).shareReplay();

function createUserStream(refreshUserStream) {
  return responseStream.map(function (users) {
    return users[Math.floor(Math.random() * 30)];
  }).merge(refreshButtonStream.map(function (ev) {
    return null;
  })).merge(refreshUserStream.withLatestFrom(responseStream, function (event, users) {
    return users[Math.floor(Math.random() * 30)];
  })).startWith(null);
}
...
user1Stream.subscribe(function (user) {
  fillUser('.user1', user);
});

user2Stream.subscribe(function (user) {
  fillUser('.user2', user);
});

function fillUser(query, user) {
  var $el = (0, _jquery2['default'])(query);
